...
def create_app(scene_state, trainer):
    ...
    from toon_template.toon_app import ToonApp
    scene_state.renderer.device = trainer.device  # Use same device for trainer and renderer
    interactive_app = ToonApp(wisp_state=scene_state, background_task=trainer.iterate, window_name="Template App")
...
if __name__ == "__main__":
    ...
    from toon_neural_field import ToonNeuralField
    from toon_template.toon_trainer import ToonTrainer
    register_class(ToonNeuralField, 'ToonNeuralField')
    register_class(ToonTrainer, 'ToonTrainer')
    ...
    # print(f"Env val:{os.environ.get('WISP_HEADLESS')}end")
    # print(f"was{os.environ.get('WISP_HEADLESS') == '1')

    if os.environ.get('WISP_HEADLESS') == '0':
        setup_cuda_context()
