...
#
# How to call: 
#   - bash experiments.sh <config_yaml> <data_dir> <results_dir>
...
data_dirs=$(eval "$data_dir_cmd" | head -n $data_dirs_count)
...
if [ -f "$progress_file" ] && [ ! "$(grep -e '<done>' $progress_file)" ]; then
    ...
    progress_file=$checkpoint_dir/$progress_file_name
    echo "Create progress file $progress_file"
    touch $progress_file
fi
...
for DATA_PATH in $data_dirs; do
    ...
    triplanar_nerf_name=$experiment_name-$data_name
    ...
    if ! grep -q $triplanar_nerf_name $progress_file; then
        ...
        echo WISP_HEADLESS=1 python3 ./toon_template/toon_main.py --config $config_path --dataset-path $DATA_PATH --dataset-num-workers 4 --log-dir $checkpoint_dir --exp-name $triplanar_nerf_name
